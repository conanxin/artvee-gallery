...
| **P8D+4B** | Media replay queue scope cleanup (2026-07-04: fixed cron wrapper passing `reports/` instead of `reports/runtime/` so `rel.startswith("replayed/")` no longer mis-matches; hardened `_scan_pending_media` with `_classify_pending_path` (active vs terminal_replayed/terminal_quarantine/results/backup_or_legacy/legacy_nested/unknown); archived `queue-fix-backup-20260703-062946/stable_dup/` + `daily-health/replayed/` + `daily-health/quarantine/` to `legacy-cleaned/20260704/` after 44-file `queue-scope-cleanup-backup-20260704-063618/`; renamed cron `outcome` from `noop_zero_pending` to `no_pending` and added the per-bucket breakdown to `cron-<date>.json`; dry-run confirmed `pending_before=0, outcome=no_pending`; 0 nested paths in active trees; integrity --strict PASS; readiness PASS) | ✅ PASS | 2026-07-04 | `<workspace>/reports/artvee-gallery-p8d4b-media-replay-queue-scope-cleanup-20260704.md` |
...
| **P9G** | Public Gallery Expansion 200 → 300 selected works. `--gallery-limit 300` via `confirm_demo_refresh.sh`; candidate QA: 300 records, 300 thumbs (256+512 each), public-safe JSON (no metadata_path/digest_path/TOKEN/SECRET/CHAT_ID); Gallery bundle 17 MB (soft 15 MB, hard 20 MB); Digest bundle 792 KB; all 7 online endpoints HTTP 200; Pages Guard PASS; rsync + commit 646abf3 pushed; Artvee commit `e07bfb5`. `GALLERY_THUMB_TARGET=200 → 300` in `confirm_demo_refresh.sh`. Docs updated: README.md, CHANGELOG.md, METRICS_MODEL.md, PROJECT_STATUS.md, ROADMAP.md, POST_STABLE_OPERATIONS.md, RELEASE_NOTES_v0.2.1.md. No download/refill/batch triggered. | ✅ PASS | 2026-07-12 | `<workspace>/reports/artvee-gallery-p9g-public-gallery-300-20260712.md` |
| **P9G+1 (audit)** | Public Gallery Performance / Bundle Budget Audit. Decomposed 14.88 MB bundle: 256=3.21 MB (22%) / 512=11.36 MB (76%) / non-image=0.31 MB. Confirmed Grid uses 256 (lazy, 300/300 cards), 512 only on detail-panel click. Desktop initial=0.875 MB/50 req, Mobile initial=0.605 MB/25 req, Full-scroll=3.52 MB. Projections: 400→19.73 MB (tight at 20 MB hard), 500→24.59 MB (over hard). P9G+2 top pick: drop 512 from public bundle. Read-only. No tracked modifications, no commits, no Pages push, no download/refill/batch. | ✅ PASS_WITH_WARN | 2026-07-12 | `<workspace>/reports/artvee-gallery-p9g1-performance-bundle-audit-20260712.md` |
| **P9G+2** | Public Gallery Bundle Optimization. Stop shipping 512 thumbs in the public bundle; bundle 14.88 MB → **3.48 MB** (–11.40 MB / –76.6%); detail panel falls back to 256 under `--detail-thumb-policy=none`. Exporter gained `--detail-thumb-policy {all,none}` (default `none`); `confirm_demo_refresh.sh` gained matching flag with policy-aware QA + 5 MB / 8 MB soft/hard budgets under `none`. `web/app.js` got explicit `thumb_512 || image_path || thumb_256` fallback + `bundle policy` row in detail-meta. `gallery_stats.json` gained `detail_thumb_policy`, `thumbs_256_count`, `thumbs_512_count`. Candidate 300 records / 300 256 thumbs / 0 512 thumbs / 0 leaks. Local HTTP + Playwright smoke: 300 cards, 0 /512/ requests, 5 detail clicks all 256, 0 broken, 0 console errors. Pages Guard: 305 changed / 305 allowed / 0 blocked. Pages commit **33ff10b** pushed (646abf3..33ff10b). Online: 300 records, 0 /512/ references, 7/7 endpoints 200, 10 sample 256s 200, sample 512s 404. Local Library 512 thumbs untouched. New `docs/PUBLIC_BUNDLE_POLICY.md`. v0.2.1 observation window restarts here. | ✅ PASS | 2026-07-12 | `<workspace>/reports/artvee-gallery-p9g2-bundle-optimization-20260712.md` |
